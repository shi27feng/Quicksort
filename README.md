...
The nub of the Quicksort algorithm is partitioning a sequence of data to smalle enogh sequence which can be sorted by single work group, in fact Quicksort algorithm on GPU has two main phases, in the first phase one kernel divide the whole sequence into small enough sequence which can be sorted by one workgroup.        
